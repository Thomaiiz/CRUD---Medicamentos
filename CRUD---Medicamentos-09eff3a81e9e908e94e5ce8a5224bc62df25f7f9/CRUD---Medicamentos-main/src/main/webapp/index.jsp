...
<body>
	<header>
		...
		<nav>
			...
			if (usuario != null) {
			%>
			<a href="CarregarMedicamentos">Medicamentos Cadastrados</a>
			<%
			}
			%>
	...
	<main style="padding: 40px; background: #fff; color: #333;">
		...
		<div
			...
			<div class="card">
				<a href="detalhes.jsp?nome=<%=java.net.URLEncoder.encode(m.getNomeComercial(), "UTF-8")%>"
					style="text-decoration: none; color: inherit;">
					<img class="card-img-top" src="img/<%=m.getImagem()%>" alt="<%=m.getNomeComercial()%>">
					<h3 class="card-title"><%=m.getNomeComercial()%></h3>
					<p class="descricao"><%=m.getDescricao()%></p>
					<p class="preco">R$ <%=String.format("%.2f", m.getPreco())%></p>
				</a>
				...
				<div class="botoes-acao">
					...
					<form action="RemoverMedicamentoServlet" method="post" style="display:inline;">
						<input type="hidden" name="nomeComercial" value="<%=m.getNomeComercial()%>">
						<button type="submit" class="botao excluir" onclick="return confirm('Tem certeza que deseja excluir?')">Excluir</button>
					</form>
				</div>
				<% } %>
			</div>
